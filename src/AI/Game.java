...
public class Game {
    ...
    private void setPlayers() {
        if (numberOfPlayers == 2 || numberOfPlayers == 4) {
            ...
            setPlayersGoalRow();
            AI = new AI(numberOfPlayers, players);
        } else {
        ...
        while (!END) {
            ...
            if (!is_AI) {
                ...
                if (numberOfPlayers == 2) {
                    AI.setMinMax(players, board, checker, connection, TURN);
                    Object AIResult = AI.findBestMove(board.getGameBoard());
                    if (AIResult instanceof Position) {
                        Position AIMove = (Position) AIResult;
                        System.out.println(AIMove);
                        changeTurn = changePawn(AIMove.getPositionDirection());
                    } else {
                        Fence AIFence = (Fence) AIResult;
                        System.out.println(AIFence);
                        changeTurn = placeFence(AIFence.getFrom(), AIFence.getTo());
                    }
                } else {
